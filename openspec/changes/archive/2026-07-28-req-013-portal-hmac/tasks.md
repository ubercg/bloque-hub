              ...
              └─→ PR #3  feat/req013-03-client-happy-path      (Slice B1 — closes RISK-4)   ✅ done
                    ...
                    └─→ PR #4  feat/req013-04-error-taxonomy-mapping  (Slice B2 — ATOMIC)   ✅ done
                          ...
                          └─→ PR #5  feat/req013-05-lead-prefill       (Slice C)   ✅ done 📍
                                │
                                └─→ PR #6  feat/req013-06-frontend-error-states (Slice D)   ✅ done
                                      │
                                      └─→ PR #7  feat/req013-07-rollout-hygiene-docs (Slice F)   ⏳ partial 📍
                                            │
...
**Payload-shape correction (2026-07-28 connected smoke):** Portal real sends Hub-canonical keys (`nombre_completo`, `correo_institucional`, …) **plus** English synonyms in the same `lead_prefill` object — NOT the obsolete local-double names (`nombre_solicitante` / `email_solicitante` / `telefono_solicitante` / `numero_invitados`). RN-019 requires the double to match that payload fidelity, not only HMAC. `portal-mock.py` and `map_lead_prefill` were corrected accordingly; obsolete double names are intentionally unread (regression test `test_obsolete_double_names_alone_do_not_hydrate`). `ciudad` / `space_id` are emitted as traps and ignored (RN-015).
...
- [x] 6.1 `features/quote-wizard/gate-error.ts` (new): `readGateError(err)` + `resolveGateFailure(status?, reason?) -> [GateStatus, string]` — pure, testable without rendering. Must not import from `app/` (a convention, **not enforced** by `.dependency-cruiser.cjs`, which has no rule constraining `features/ → app/` — do not claim tooling coverage that does not exist, design §7).
- [x] 6.2 `resolveGateFailure` precedence (design §7): `reason` match first (authoritative) → status-code fallback (`422`/`403`/`503`/`502`) → `GATE_STATUS.UNKNOWN_ERROR` + `GENERIC_ERROR_MESSAGE` last, **never** `not_eligible` — this is the fix for the pre-existing `page.tsx:60-61` mislabeling defect.
- [x] 6.3 Store: `GATE_STATUS` const-object + extracted `GateStatus` type (const-object + extracted-type pattern per the TypeScript skill), adding `AUTH_FAILURE` and `UNKNOWN_ERROR` members.
- [x] 6.4 Store: new state `leadPrefill`, `fechaTentativa`, `espacioRequerido` + `hydrateFromPrefill(prefill)` bulk action in **one** `set(...)` call. Rules: only non-`null` incoming values are written (a `null` leaves the current value untouched, RN-013, and re-hydration cannot clobber text the applicant already typed); `tipo_evento_sugerido`/`como_conociste_bloque` applied only if the value is a member of the corresponding options constant (a non-member is dropped silently, RN-014 says that is normal); it **never** writes `descripcionEvento` (RN-021 made structural on the frontend too); the three new fields are added to `initialState` so `reset()` clears them.
- [x] 6.5 `solicitud/page.tsx`: rewrite the branching block at `:49-62` to call `resolveGateFailure(status, detail?.reason)`; on success, `hydrateFromPrefill(data.lead_prefill)` runs **before** `setGateStatus(UNLOCKED)` and `router.push`, so Step 3 mounts already filled.
- [x] 6.6 `page.tsx`: `AUTH_FAILURE_MESSAGE` copy exactly per design §7 / Phase 0.3 (system fault, no call to action, no claim that an alert was raised — there is no alerting per §2.3).
- [x] 6.7 `StepEspacio.tsx`: read-only informational `<p>` block rendered when `fechaTentativa` or `espacioRequerido` is non-null — not bound to any input. Explicit prohibition: no effect may feed these into `addItem`, a date default, or a `space_id` lookup (RN-015, RN-022).
- [x] 6.8 Confirm `StepSolicitante.tsx` needs **no change** — it is already controlled from the same store keys `hydrateFromPrefill` writes. Do not add an effect there.
- [x] 6.9 Playwright e2e (no unit runner exists, design §12): auth-failure copy visible with no retry CTA and textually distinct from the not-eligible and Portal-unavailable messages; unknown-error scenario does **not** render folio-rejection copy (regression test for the `:60-61` defect); success → Step 3 pre-filled and editable; Step 2 shows the reference block with no date/space preselected; `status_label` never rendered anywhere via `page.route()` stubbing a `status_label` value and asserting its absence from the DOM.
...
- [x] 7.1 Secret-hygiene check #1 (§9): test asserting `FolioValidateResponse.model_fields` and `LeadPrefillOut.model_fields` contain no key matching `api_key|secret|signature`. *(`tests/test_portal_gate_secret_hygiene.py::TestSecretHygieneModelFields`, 2 tests, green.)*
- [x] 7.2 Secret-hygiene check #2: test asserting the configured secret is not a substring of the serialized body on every gate/submit path, success and error. *(Same file, `TestSecretHygieneGateResponseBody` (5 cases: eligible+full prefill, not-eligible, unavailable, auth-failure, invalid-format) + `TestSecretHygieneSubmitResponseBody` (4 cases: happy-path 201, not-eligible 403, auth-failure 502, payload-validation 422) — 9 tests, all green.)*
- [x] 7.3 Secret-hygiene check #3 — **the one the DoD actually asks for**: `npm run build` in `src/frontend`, then grep the emitted `.next/static` and `.next/server` output for the secret value and the literals `PORTAL_HUB_API_SECRET` / `PORTAL_HUB_API_KEY` — zero hits required. Run in CI after the build step, or record as a manual release check in the bitácora if CI has no frontend build stage. *(No `.github/workflows/` exists in this repo — no CI build stage to hook. Ran locally: `npm run build` succeeded (Next.js 16.1.6, Turbopack, 27 routes), then grepped `.next/static` + `.next/server` for `dev-local-portal-hub-secret`, `PORTAL_HUB_API_SECRET`, `PORTAL_HUB_API_KEY` — zero hits on all three. Recorded as a manual pre-deploy release check in BIT-021, since there is no FE build stage in CI to automate it against.)*
- [x] 7.4 **Connected credentials loaded for smoke against Portal real** (2026-07-28). `PORTAL_HUB_API_KEY`/`SECRET` + `PORTAL_API_BASE_URL=https://bloqueqro-api.municipiodequeretaro.gob.mx` live in gitignored `.env` (never in tracked compose). RN-020 second pair for a distinct staging env still pending when that env exists — this closes the smoke gate, not the full multi-env rollout checklist forever.
- [x] 7.5 **Connected smoke against real Portal** (2026-07-28): folio `BCE-20260717-121058-4083` → unlock + prefill; negative smoke with wrong secret → `502 INTEGRATION_AUTH_FAILURE`. Prefill alias fix required: Portal sends Hub-shaped keys (`nombre_completo`, …) + English synonyms, not the local-double names alone.
- [x] 7.6 Repo-wide grep sweep (final gate before the tracker merges to `main`): zero hits for `space-event-requests`, `PORTAL_API_KEY`, `X-Api-Key`; `status_label` appears only in `client.py`'s log call and `portal-mock.py` (full §10 removals table). *(Ran with `rg` excluding `node_modules`/`.next`/`.git`/`__pycache__` across `src/`, `portal-mock.py`, both compose files, `.env.example`. `space-event-requests`: 3 hits, all in tests asserting its ABSENCE (`test_portal_mock.py:208` — old route now 404s; `test_portal_gate_client.py:540,571` — asserts not in built URL). `PORTAL_API_KEY`/`X-Api-Key`: 3 hits, all in `test_config.py`'s own grep-style regression test (task 4.7) — zero hits in live `src/` code, `.env.example`, or compose files. `status_label`: hits only in `portal-mock.py` (3, sending it as Portal would) and `client.py:147,150` (the diagnostic log call) plus two frontend e2e specs that stub/assert its absence from the DOM — exactly the §10 row 3 exception set, nothing outside it.)*
- [x] 7.7 **Living documentation** (mandatory, `.cursorrules`/`CLAUDE.md`/`AGENTS.md` rule — Obsidian vault, not this repo): rewrite `30-API/API-024-*.md` for the new contract and `lead_prefill`; tick REQ-013 DoD checkboxes in `10-Requerimientos/`; update `20-Arquitectura/ARQ-001-Decisiones-Core.md` with `PORTAL_HUB_API_KEY`/`SECRET`, `REQUERIMIENTOS_ESPECIALES_MAX_LENGTH`, and the retirement of `PORTAL_API_KEY`; close RISK-4 in REQ-012's risk log; new `50-Bitacora/BIT-XXX-Estatus-REQ-013.md` entry with implementation summary, local smoke-test results, and anything still pending (e.g. unresolved 0.2 proxy verification, credential delivery status). *(Done — see BIT-021-Estatus-REQ-013.md.)*

**Phase 7 status:** 7/7 done for the smoke gate (7.1–7.7). RN-020 second credential pair for a dedicated staging env and Sigao edge-proxy verification remain operational follow-ups, not code blockers for this change.
