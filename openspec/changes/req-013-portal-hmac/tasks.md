...
- [ ] 2.6 `docker-compose.override.yml`: add `PORTAL_HUB_API_KEY` / `PORTAL_HUB_API_SECRET` to **both** the `backend` and `portal-mock` services with matching, obviously-fake dev values, plus a comment stating these are local-double-only credentials and RN-020 forbids reusing them anywhere else.
